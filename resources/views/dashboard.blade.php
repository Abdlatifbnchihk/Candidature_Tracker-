<x-app-layout>
    <div class="flex min-h-screen w-full bg-[#F8F9FC]">
        ...
        <aside
            class="w-20 bg-white border-r border-slate-200/80 flex flex-col items-center justify-between py-6 shrink-0 z-20">
            <div class="flex flex-col items-center gap-8 w-full">
                <div
                    class="w-10 h-10 bg-[#7C3AED] rounded-xl flex items-center justify-center text-white shadow-lg shadow-purple-500/20">
                ...
                <nav class="flex flex-col items-center gap-4 w-full px-2">
                    <a href="{{ route('dashboard') }}"
                        class="w-12 h-12 bg-[#F3E8FF] text-[#7C3AED] rounded-xl flex items-center justify-center transition-all duration-200">
                    ...
                    <a href={{ route('applications.index') }}
                        class="w-12 h-12 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-xl flex items-center justify-center transition-all duration-200">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M21 13a1 1 0 01-1 1H4a1 1 0 01-1-1V5a1 1 0 011-1h16a1 1 0 011 1v8zM12 11V3m0 8l-4-4m4 4l4-4" />
                        </svg>
                    </a>
                    <a href="#"
                        class="w-12 h-12 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-xl flex items-center justify-center transition-all duration-200">
            ...
            <div class="flex flex-col items-center gap-5 w-full px-2">
                <a href="{{ route('profile.edit') }}"
                    class="w-12 h-12 text-slate-400 hover:text-slate-600 rounded-xl flex items-center justify-center transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div
                    class="w-10 h-10 rounded-full bg-purple-600 text-white flex items-center justify-center text-xs font-bold uppercase shadow-sm select-none">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="h-16 bg-white border-b border-slate-200/80 px-8 flex items-center justify-between z-10">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="inline-block m-0 p-0">
                        @csrf
                        <button type="submit"
                            class="bg-[#7C3AED] hover:bg-[#6D28D9] text-white font-semibold text-sm px-4 py-2 rounded-xl shadow-md shadow-purple-500/10 flex items-center gap-2 transition duration-150 ease-in-out border-0 cursor-pointer">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.2"
            ...
            <main class="flex-1 p-8 max-w-[1400px] w-full mx-auto space-y-6">
                ...
                <section
                    ...
                    <div class="max-w-xl space-y-4 relative z-10">
                        ...
                        <p class="text-slate-600 text-sm sm:text-base font-medium leading-relaxed">
                            ...
                            heights with your dedicated platform.
                        </p>
                        <form action="applications.index" class="flex items-center gap-3 pt-2">
                            <button type="submit"
                                class="bg-[#7C3AED] hover:bg-[#6D28D9] text-white text-xs font-bold px-5 py-2.5 rounded-xl shadow-lg shadow-purple-500/10 transition">
                                Get Started
                            </button>
                        </form>
                    </div>
                    ...
                    <div
                        class="hidden lg:block relative shrink-0 z-10 w-44 h-28 bg-white rounded-2xl border border-purple-100/50 p-4 shadow-xl shadow-purple-950/5 transform translate-y-2">
                        <div class="flex items-center justify-between">
                            <span
                                class="w-6 h-6 bg-emerald-50 text-emerald-600 rounded-lg flex items-center justify-center">
                        ...
                        <span
                            class="absolute -top-1.5 -left-1.5 w-3 h-3 bg-purple-400 rounded-full animate-pulse"></span>
                    </div>
                </section>
                ...
                <section class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div class="bg-white border border-slate-200/60 p-6 rounded-2xl shadow-sm flex items-center gap-4">
                        ...
                        <div>
                            <span
                                class="text-xs font-semibold text-slate-400 block tracking-wide uppercase">Candidatures
                                Actives</span>
                            <div class="flex items-baseline gap-1.5 mt-0.5">
                                <span class="text-2xl font-bold text-slate-900">{{ $activeApplications }}</span>
                                <span
                                    class="text-[11px] font-bold text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded">En
                                    cours</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border border-slate-200/60 p-6 rounded-2xl shadow-sm flex items-center gap-4">
                        ...
                        <div>
                            <span class="text-xs font-semibold text-slate-400 block tracking-wide uppercase">Entretiens
                                Planifiés</span>
                            <div class="flex items-baseline gap-1.5 mt-0.5">
                                <span class="text-2xl font-bold text-slate-900">{{ $upcomingInterviews }}</span>
                                <span class="text-[11px] font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded">À
                                    passer</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border border-slate-200/60 p-6 rounded-2xl shadow-sm flex items-center gap-4">
                        ...
                        <div>
                            <span class="text-xs font-semibold text-slate-400 block tracking-wide uppercase">Taux de
                                réponse</span>
                            <div class="flex items-baseline gap-1.5 mt-0.5">
                                <span class="text-2xl font-bold text-slate-900">{{ $responseRate }}%</span>
                                <span
                                    class="text-[11px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded">Global</span>
                            </div>
                        </div>
                    </div>
                ...
                <section class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                    ...
                    <div
                        class="lg:col-span-2 bg-white border border-slate-200/60 rounded-2xl shadow-sm overflow-hidden">
                        <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                            ...
                            <a href="#" class="text-xs font-bold text-purple-600 hover:text-purple-700 transition">View
                                all</a>
                        </div>

                        <div class="divide-y divide-slate-100/70">
                            <div
                                class="p-4 px-6 flex items-center justify-between hover:bg-slate-50/50 transition duration-150">
                                <div>
                                ...
                                <div class="flex items-center gap-8">
                                    <span
                                        class="px-2.5 py-1 text-[11px] font-bold text-blue-600 bg-blue-50 border border-blue-100/50 rounded-lg">Interview</span>
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-8">
                                    <span
                                        class="px-2.5 py-1 text-[11px] font-bold text-amber-600 bg-amber-50 border border-amber-100/50 rounded-lg">In
                                        progress</span>
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-8">
                                    <span
                                        class="px-2.5 py-1 text-[11px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100/50 rounded-lg">Offer</span>
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-8">
                                    <span
                                        class="px-2.5 py-1 text-[11px] font-bold text-rose-600 bg-rose-50 border border-rose-100/50 rounded-lg">Refused</span>
                        ...
                        <div
                            class="bg-slate-50 p-4 border-t border-slate-100 text-center flex items-center justify-center gap-2 text-xs font-semibold text-slate-400">
                    ...
                    <div class="space-y-6">
                        <div class="bg-white border border-slate-200/60 rounded-2xl shadow-sm p-6 space-y-4">
                            ...
                            <div class="space-y-3">
                                ...
                                <p class="text-xs text-slate-500 font-medium leading-relaxed">
                                    Graduates who list explicit open-source projects or live web deployments increase
                                    their initial technical response rates by up to 15%.
                                </p>
                            </div>

                            <div class="space-y-2 pt-2">
                                <a href="#"
                                    class="w-full bg-slate-50 hover:bg-slate-100 p-3 rounded-xl border border-slate-200/40 flex items-center justify-between text-xs font-bold text-slate-700 transition">
                        ...
                        <div
                            class="bg-white border border-slate-200/60 rounded-2xl shadow-sm p-6 flex flex-col items-center text-center justify-center min-h-[160px] relative overflow-hidden">
                            <div
                                class="w-10 h-10 bg-slate-50 border border-slate-200/60 text-slate-400 rounded-full flex items-center justify-center font-bold text-lg mb-3">
                                <span>+</span>
                            </div>
            ...
            <footer
                class="mt-auto border-t border-slate-200/80 bg-white px-8 py-4 flex flex-col sm:flex-row items-center justify-between text-[11px] font-semibold text-slate-400 gap-3">
                <span>&copy; {{ date('Y') }} CandidatureTracker. Made with passion for the future talent
                    ecosystem.</span>
