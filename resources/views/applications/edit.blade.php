        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <main class="flex-1 p-8 max-w-[1000px] w-full mx-auto space-y-6">
                <div class="flex items-center gap-2 text-xs font-bold tracking-wide text-slate-400 uppercase">
                    <a href="{{ route('dashboard') }}" class="hover:text-[#7C3AED] transition">Dashboard</a>
                    <span>/</span>
                    <span class="text-slate-500">{{ $application->company }}</span>
                    <span>/</span>
                    <span class="text-[#7C3AED]">Modifier</span>
                </div>

                <div class="space-y-1">
                    <h1 class="text-2xl font-extrabold text-[#1E1B4B] tracking-tight">Modifier le suivi</h1>
                    <p class="text-slate-500 text-sm font-medium">Mettez à jour les informations de votre opportunité chez <span class="text-[#7C3AED] font-semibold">{{ $application->company }}</span>.</p>
                </div>
                    ...
                    <form method="POST" action="{{ route('applications.update', $application->id) }}" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                <input type="text" name="company" id="company" value="{{ old('company', $application->company) }}" required placeholder="Ex: Stripe, Vercel" 
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="position" id="position" value="{{ old('position', $application->position) }}" required placeholder="Ex: Développeur Fullstack" 
                                    class="w-full bg-slate-50 text-sm text-slate-800 placeholder-slate-400 px-4 py-3 border border-slate-200/80 rounded-xl focus:bg-white focus:ring-4 focus:ring-purple-500/10 focus:border-[#7C3AED] transition">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                ...
                                @error('status') <p class="text-xs text-red-500 mt-1 font-semibold">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                @error('priority') <p class="text-xs text-red-500 mt-1 font-semibold">{{ $message }}</p> @enderror
                            </div>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label for="url" class="text-xs font-bold text-slate-500 uppercase tracking-wider">Lien de l'offre</label>
                                <input type="url" name="url" id="url" value="{{ old('url', $application->url) }}" placeholder="https://..." 
                                    class="w-full bg-slate-50 text-sm text-slate-800 placeholder-slate-400 px-4 py-3 border border-slate-200/80 rounded-xl focus:bg-white focus:ring-4 focus:ring-purple-500/10 focus:border-[#7C3AED] transition">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="date" name="applied_at" id="applied_at" value="{{ old('applied_at', $application->applied_at ? $application->applied_at->format('Y-m-d') : '') }}" 
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="notes" id="notes" rows="4" placeholder="Détails, noms des recruteurs, questions préparées..." 
                                class="w-full bg-slate-50 text-sm text-slate-800 placeholder-slate-400 px-4 py-3 border border-slate-200/80 rounded-xl focus:bg-white focus:ring-4 focus:ring-purple-500/10 focus:border-[#7C3AED] transition">{{ old('notes', $application->notes) }}</textarea>
                            @error('notes') <p class="text-xs text-red-500 mt-1 font-semibold">{{ $message }}</p> @enderror
                        </div>

                        <div class="space-y-2">
                            <label for="file" class="text-xs font-bold text-slate-500 uppercase tracking-wider">Remplacer le document joint (Optionnel)</label>
                            
                            @if($application->file_path)
                                <div class="flex items-center gap-2 bg-purple-50 border border-purple-100 px-4 py-2.5 rounded-xl mb-2 text-xs font-semibold text-[#7C3AED]">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                    <span>Document actuel :</span>
                                    <a href="{{ asset('storage/' . $application->file_path) }}" target="_blank" class="underline hover:text-[#6D28D9] transition">
                                        Voir le document enregistré
                                    </a>
                                </div>
                            @endif

                            <div class="flex items-center justify-center w-full">
                                <label class="flex flex-col w-full h-28 border-2 border-dashed border-slate-200 rounded-xl hover:bg-slate-50 hover:border-[#7C3AED]/30 transition cursor-pointer group">
                                    <div class="flex flex-col items-center justify-center pt-5">
                                        <svg class="w-7 h-7 text-slate-300 group-hover:text-[#7C3AED]/50 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                                        <p class="pt-1 text-xs font-bold text-slate-400 group-hover:text-slate-500 transition">Cliquez pour téléverser un nouveau fichier (.pdf, .docx)</p>
                                    </div>
                        ...
                        <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                            <a href="{{ route('dashboard') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold text-xs px-6 py-3 rounded-xl transition duration-150">
                                Annuler
                            </a>
                            <button type="submit" class="bg-[#7C3AED] hover:bg-[#6D28D9] text-white font-bold text-xs px-8 py-3 rounded-xl shadow-lg shadow-purple-500/10 flex items-center gap-2 transition duration-150">
                                ...
                                <span>Mettre à jour la candidature</span>
                            </button>
                        </div>
                    </form>
                </div>
